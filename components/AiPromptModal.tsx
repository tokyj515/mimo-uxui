...
const AiPromptModal: React.FC<AiPromptModalProps> = ({
    ...
    const builtPrompt = useMemo(() => {
        ...
        // 4. 메시지 타입: auto일 땐 아예 섹션을 안 넣음(서버 기본 MMS 적용)
        if (sendTypePref !== "auto") {
            lines.push("\n[메시지 타입]");
            lines.push(`- 메시지 타입: ${sendTypePref}`);
        }
        if (slideCountHint && sendTypePref === "RCS_CAROUSEL") {
            lines.push(`- RCS 캐러셀 카드 수: ${slideCountHint}장`);
        }

        // 5. 언어/톤: ✅ enabledLangs 기반
        lines.push("\n[언어/톤]");
        lines.push(`- 사용할 언어 코드: ${enabledLangs.join(", ")}`);
        if (tone) lines.push(`- 메시지 톤 & 스타일: ${tone}`);
    ...
    const handleSubmitClick = () => {
        // aiPrompt가 비어있으면 builtPrompt를 기본값으로 채우고 요청
        if (!aiPrompt.trim()) setAiPrompt(builtPrompt);
        onSubmit();
    };
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                placeholder="예) 연말을 맞아 VIP 고객에게 감사 인사와 혜택을 안내하려고 해."
                                value={campaignGoal}
            ...
            case 5:
                return (
                    <div className="space-y-3">
                        ...
                        <p className="text-[11px] text-slate-500">언어는 버튼으로 선택해 주세요.</p>
            ...
            default:
                return (
                    <div className="space-y-3">
                        ...
                        <p className="text-[11px] text-slate-500">
                            혜택/유의사항/수신거부 문구 등 빠지면 안 되는 요소를 적어 두면 좋아요.
                        </p>
                        ...
                        return (
                            <div
                                ...
                                className={`flex-1 h-1.5 rounded-full ${
                                    active ? "bg-teal-500" : done ? "bg-teal-200" : "bg-slate-200"
                                }`}
                            />
                        );
                    })}
            ...
            {step} / {totalSteps}
                ...
                <div className="space-y-2">
                    ...
                    <textarea
                        ...
                        placeholder="여기에서 세부 문장을 직접 다듬을 수 있어요."
                        value={aiPrompt}
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-2 text-[11px] text-slate-400 sm:max-w-xs">
                        <span>실제 발송 전에는 반드시 한 번 더 검토해 주세요.</span>
                    </div>
