...
const AiPromptModal: React.FC<AiPromptModalProps> = ({
    ...
    const [purpose, setPurpose] = useState<"" | "공지" | "이벤트" | "알림" | "기타">("");
    ...
    // 5단계: 언어 / 톤  ✅ 버튼 선택형으로 변경
    const [selectedLangs, setSelectedLangs] = useState<string[]>(["ko"]); // ko 고정
    const [tone, setTone] = useState(""); // 예: 정중하지만 너무 딱딱하진 않게, 등

    // 6단계: 필수 포함 요소
    const [mustInclude, setMustInclude] = useState("혜택 요약, 상세 안내, 유의사항, 문의처를 꼭 넣어줘.");
    const [legalNote, setLegalNote] = useState("(광고) 태그와 무료수신거부 번호 문구도 빠뜨리지 말고 넣어줘.");
    ...
    const builtPrompt = useMemo(() => {
        ...
        // 5. 언어/톤  ✅ selectedLangs 기반
        lines.push("\n[언어/톤]");
        lines.push(`- 사용할 언어: ${selectedLangs.join(",")} (콤마 구분, 예: ko 또는 ko,en 등)`);
        if (tone) {
        ...
        lines.push("\n위 정보를 바탕으로, 실제 발송 가능한 수준의 메시지 초안을 만들어줘.");
    ...
    const renderStep = () => {
        switch (step) {
            case 1:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">1단계. 어떤 상황/캠페인에 쓰는 메시지인가요?</h4>
                        <p className="text-[11px] text-slate-500">예) 연말 VIP 고객 감사, 미납요금 안내, 신규 요금제 런칭 등</p>

                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">캠페인/행사명 (선택)</label>
                            <input
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">목적/상황 (필수)</label>
                            <textarea
            ...
            case 2:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">2단계. 누구에게, 어느 정도 규모로 보내나요?</h4>
                        <p className="text-[11px] text-slate-500">대상 특징과 대략적인 발송량을 알려주면 톤과 구성에 도움이 돼요.</p>

                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">발송 대상 (필수)</label>
                            <textarea
                                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm min-h-[80px] resize-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">예상 발송량 (선택)</label>
                            <input
                                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
            ...
            case 3:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">3단계. 광고성인가요, 안내성인가요?</h4>
                        <p className="text-[11px] text-slate-500">광고 여부와 발송 목적에 따라 문구 구성과 필수 표현이 달라져요.</p>

                        <div className="space-y-2">
                            <span className="text-[11px] font-medium text-slate-700">광고 여부</span>
                        ...
                        <div className="space-y-2">
                            <span className="text-[11px] font-medium text-slate-700">발송 목적</span>
                            <div className="flex flex-wrap gap-2 text-xs">
                                {["공지", "이벤트", "알림", "기타"].map((p) => (
                                    <button
                                        key={p}
            ...
            case 4:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">4단계. 어떤 메시지 타입이 더 좋을까요?</h4>
                        <p className="text-[11px] text-slate-500">
                            대략 선호하는 형태가 있다면 알려주고, 애매하면 &quot;상황에 맞게 알아서&quot;를 선택해 주세요.
                        </p>

                        <div className="space-y-2">
                            <span className="text-[11px] font-medium text-slate-700">타입 선호</span>
                            <div className="flex flex-wrap gap-2 text-xs">
                                {[
                                    { code: "auto", label: "상황에 맞게 알아서" },
                                    { code: "SMS", label: "SMS" },
                                    ...
                                    <button
                                        key={opt.code}
                                        type="button"
                                        onClick={() => setSendTypePref(opt.code as SendTypePreference)}
                                        className={`h-8 px-3 rounded-full border transition ${
                        ...
                        {sendTypePref === "RCS_CAROUSEL" && (
                            <div className="space-y-2">
                                <span className="text-[11px] font-medium text-slate-700">RCS 캐러셀 카드 수 (선택)</span>
                                <div className="flex flex-wrap gap-2 text-xs">
                                    {["2", "3", "4", "5"].map((n) => (
                                        <button
                                            key={n}
            ...
            case 5:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">5단계. 언어와 톤을 어떻게 할까요?</h4>
                        <p className="text-[11px] text-slate-500">언어는 버튼으로 선택해 주세요. (ko는 기본으로 항상 포함돼요)</p>

                        {/* ✅ 언어 선택 버튼 */}
                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">사용할 언어</label>

                            <div className="flex flex-wrap gap-3 items-center text-xs">
                                {LANGS.map((lang) => {
                                    const enabled = selectedLangs.includes(lang.code);
                                    return (
                                        <button
                                            key={lang.code}
                                            type="button"
                                        ...
                                        >
                                            ...
                                            {lang.code === "ko" && <span className="text-[10px] text-slate-500">(기본)</span>}
                                        </button>
                                    );
                                })}
                            </div>
                            ...
                            <p className="text-[11px] text-slate-500">
                                선택된 언어: <b>{selectedLangs.join(", ")}</b>
                            </p>
                        </div>

                        {/* 톤 */}
                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">메시지 톤 & 스타일</label>
                            <textarea
                                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm min-h-[80px] resize-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                                placeholder="예) 부담스럽지 않게, 혜택 + 간단 행동 중심. 너무 길게 늘어지지 않되 성의 있게."
                                value={tone}
            ...
            default:
                return (
                    <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-900">6단계. 꼭 들어가야 하는 내용이 있다면 알려주세요.</h4>
                        <p className="text-[11px] text-slate-500">혜택/유의사항/수신거부 문구 등 빠지면 안 되는 요소를 적어 두면 좋아요.</p>

                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">필수 포함 요소</label>
                            <textarea
                                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm min-h-[80px] resize-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-medium text-slate-700">준수해야 할 문구/규칙</label>
                            <textarea
                                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm min-h-[80px] resize-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
    ...
    return (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40">
            <div className="w-full max-w-3xl rounded-2xl bg-white shadow-2xl px-8 py-6 space-y-6">
                ...
                <div className="flex items-start justify-between">
                    <div className="flex gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-teal-50 text-xl">✨</div>
                        <div>
                            <h3 className="text-sm font-semibold text-slate-900">AI로 메시지 자동 작성</h3>
                            <p className="mt-1 text-[11px] text-slate-500 leading-relaxed">
                                간단한 질문에만 답하면 캠페인 목적·대상·타입까지 반영한 프롬프트를 자동으로 만들어 드릴게요.
                                필요하면 마지막에 직접 문장을 더 고쳐 쓸 수도 있어요.
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-2 text-[11px]">
                    {Array.from({ length: totalSteps }).map((_, idx) => {
                        ...
                        return (
                            <div
                                key={current}
                                className={`flex-1 h-1.5 rounded-full ${active ? "bg-teal-500" : done ? "bg-teal-200" : "bg-slate-200"}`}
                            />
                        );
                    })}
            ...
            {step} / {totalSteps}
                ...
                {/* 단계별 질문 */}
                <div className="border rounded-xl border-slate-200 bg-slate-50/60 px-4 py-4">{renderStep()}</div>

                {/*/!* 예시 프롬프트 *!/*/}
                {/*<div className="space-y-2">*/}
                {/*    <div className="flex items-center justify-between">*/}
                {/*        <span className="text-[11px] font-medium text-slate-700">예시 프롬프트로 바로 시작하기</span>*/}
                {/*        <span className="text-[10px] text-slate-400">아래 예시를 누르면 프롬프트 영역에 바로 채워집니다.</span>*/}
                {/*    </div>*/}
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <label className="text-sm font-semibold text-slate-700">최종 프롬프트</label>
                            <button
                                type="button"
                        ...
                        <span className="text-[11px] text-slate-400">{aiPrompt.length}자</span>
                    </div>
                    <textarea
            ...
            <span>
              AI가 결정한 메시지 타입과 대체 MMS 설정까지 화면에 바로 반영됩니다. 실제 발송 전에는 반드시 한 번 더 검토해 주세요.
            </span>
                    </div>
                    <div className="flex justify-end gap-2 text-xs">
                        <Button type="button" variant="outline" className="px-3" onClick={goPrev} disabled={step === 1}>
                            이전
                        </Button>
                        <Button type="button" variant="outline" className="px-3" onClick={goNext} disabled={step === totalSteps}>
                            다음
                        </Button>
                        <Button type="button" className="px-4" onClick={handleSubmitClick} disabled={aiLoading}>
